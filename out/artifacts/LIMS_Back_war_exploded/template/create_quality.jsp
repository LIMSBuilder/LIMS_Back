...
<%
    ...
    for (int i = 0; i < itemProjectList.size(); i++) {
        ...
        int balance = Sample.sampleDao.find("SELECT s.* FROM `db_sample` s ,`db_sample_project` p WHERE p.sample_id = s.id AND p.item_project_id=" + itemProjectList.get(i).get("id") + "AND balance is not null").size();
    ...
    //打开文件
    poCtrl1.webOpen("/doc/QualityTemplate.doc", OpenModeType.docNormalEdit, "张三");
